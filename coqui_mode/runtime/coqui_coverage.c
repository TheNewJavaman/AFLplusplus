 ...
 *
 * Inner loop reuses the chunked-classify primitive: each 256 B region
 * is 32 u64s = 4 cacheline chunks of 8 u64s. Chunk-OR skip + prefetch
 * pattern matches __coqui_classify_counts_and_sig exactly so output and
 * FNV-1a fold are byte-identical to the non-sparse chunked version. */
u32 __coqui_classify_counts_and_sig_sparse(u8 *map, u8 *touch) {
    ...
    for (u32 sw = 0; sw < 4; sw++) {
        ...
        while (tw) {
            ...
            /* Walk the 32 u64s in this region as 4 cacheline chunks of 8
             * u64s. Byte-identical to the chunked non-sparse path: same
             * per-chunk load-OR-skip, same prefetch pattern (guarded so
             * we never prefetch past the end of cov_map), same per-word
             * FNV-1a fold using absolute index i. */
            for (u32 k = 0; k < 32; k += 8) {
                u32 i = base + k;
                u64 w0 = m64[i+0], w1 = m64[i+1], w2 = m64[i+2], w3 = m64[i+3];
                u64 w4 = m64[i+4], w5 = m64[i+5], w6 = m64[i+6], w7 = m64[i+7];

                /* Prefetch next chunk only if it lies within cov_map
                 * (n_chunks = 8192, so i+8 < 8192 is the bound). */
                if (i + 8 < (COQUI_COV_MAP_SIZE / 8)) {
                    asm volatile("prefetch.global.L1 [%0];"
                                 :: "l"((const void *)&m64[i+8]));
                }
