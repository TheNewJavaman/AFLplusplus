...
| `bzip2_fuzzer_cpu`    | Symlink to `/tmp/coqui-bzip2-cpu/bzip2_decompress_fuzzer` |
...
- `bz2_assert_stub.c` — local replacement for coqui's `bz2_assert_stub.c`:
  ...
  (coqui mode's `ExternalSymbolGatekeeper` only allows `__coqui_*`/`__llvm_*`
...
- **`abort()` isn't in coqui mode's external allowlist.** The upstream
  `bz2_assert_stub.c` uses `abort()` in `BZ_NO_STDIO` mode — coqui mode's
...
- **`nix build` flags that coqui-cc doesn't accept.** The nix spec
  ...
  are not options on coqui mode's `coqui-cc`: heap is derived at runtime
...
- **"Mistyped AFL environment variable: AFL_COQUI_CUBIN" warning.**
  Harmless. coqui mode's env-validator table in `afl-fuzz-state.c` doesn't
...
```bash
rm -rf bzip2_fuzzer* seeds dict out .bzip2_fuzzer.build /tmp/coqui-bzip2-cpu
./build.sh
```
